...
When a Account Domain Control for `example.com` exists, anyone attempting to
...
When a Account Domain Control for `example.com` exists, signing up for a new
account with an email address on `example.com` will be disallowed. Users will
be redirected instaed to log in with the SSO IdPs configured on your existing
account.

### Account Creation

When a Account Domain Control for `example.com` exists, any ngrok users with
...
allowed change their email address to one that doesn't match your domain
control.

### Retroactive Account Consolidation

Creating a Account Domain Control does not retroactively consolidate existing
accounts created by users with matching email addresses. If you require account
consolidation as part of creating a Account Domain Control, please [contact
