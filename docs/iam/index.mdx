...
- Configure single sign-on (SSO), SCIM to federate identity and provisioning to your own IdP
...
- [**Users**](/iam/users/): An Account contains one or more **Users**. Users are members of
  ...
  or making API requests. Users may be members of multiple accounts.
- [**Bot Users**](/iam/bot-users): Accounts also contain **Bot Users** which are like Users but
  ...
  Accounts'.
- **Principals**: A principal is either a User or Bot User. Principals are
...
- [**Authtokens**](/iam/users/#credentials): Principals begin Agent sessions and create Endpoints by
  authenticating with Authtoken.
- [**API Keys**](/iam/users/#credentials): Principals make API Requests by authenticating with an API Key.
- [**SSH Public Keys**](/iam/users/#credentials): Principals create Endpoints via the SSH Reverse Tunnel
...
- [**Account Domain Controls**](/iam/domain-controls/): Account Domain Controls are used to create
  policy on Users who log in or signup with a given email domain.
