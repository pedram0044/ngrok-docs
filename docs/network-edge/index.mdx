...
When you use resolve any of ngrok's IPs via DNS, you will receive a partial
list of IPs because ngrok uses [global server load
balancing](/network-edge/gslb/), which means that DNS queries will return
different sets of IPs depending on where you are located in the world.
...
for a custom agent ingress adress](/agent/ingress/#dedicated-ips).
