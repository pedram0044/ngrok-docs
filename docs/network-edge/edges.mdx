...
Modules are middleware that enable you manipulate traffic to your applications.
Modules enable to add authentication, resiliency, acceleration, transformation,
security and observability to the traffic that transits through to your
applications.

## Backends

After traffic to an Edge has been processed by the Modules, it is sent to a
Backend. A Backend is also a Module, it is just the last Module to be handled
which determines where the traffic is sent to. There are multiple types of
